    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-7">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-widest">{dayOfWeek}, {dateStr}</p>
        ...
        {data.items.map((item) => {
          ...
          return (
            <div
              ...
              className={`bg-gray-800 rounded-2xl border shadow-sm overflow-hidden ${
                isResolved || state.committed ? 'border-emerald-600 opacity-70' : 'border-gray-700'
              }`}
            >
              {/* Card header */}
              <div className={`px-5 py-4 border-b ${
                topFlag.flag_type === 'red' ? 'bg-red-950 border-red-900' :
                topFlag.flag_type === 'amber' ? 'bg-amber-950 border-amber-900' :
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <Link href={`/profiles/${item.customer.id}`} className="hover:underline">
                      <h2 className="text-base font-semibold text-white">{item.customer.company_name}</h2>
                    </Link>
              ...
              {item.open_commitment && !state.committed && (
                <div className={`px-5 py-2.5 border-b flex items-center gap-2 ${
                  ...
                  <svg className={`w-3.5 h-3.5 flex-shrink-0 ${item.open_commitment.is_overdue ? 'text-red-400' : 'text-blue-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                  </svg>
                  <p className={`text-xs ${item.open_commitment.is_overdue ? 'text-red-200' : 'text-blue-200'}`}>
              ...
              <div className="px-5 py-4">
                ...
                {isResolved ? (
                  <div className="flex items-center gap-2.5 py-1">
                    <div className="w-5 h-5 bg-emerald-900 rounded-full flex items-center justify-center flex-shrink-0">
                      <svg className="w-3 h-3 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <p className="text-sm text-gray-300">Marked as resolved — great work.</p>
                  </div>

                /* ── Committed (new action logged) ── */
                ) : state.committed ? (
                  <div className="flex items-start gap-3 py-1">
                    <div className="w-5 h-5 bg-emerald-900 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                      <svg className="w-3 h-3 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-white">{state.committed.action}</p>
                      <p className="text-xs text-gray-400 mt-0.5">
                ...
                /* ── Normal briefing + action picker / commit form ── */
                  ...
                  <>
                    ...
                    {state.outcomeRecorded && item.pending_outcome && (
                      <div className="mb-3 flex items-center gap-2 text-xs text-gray-400 bg-gray-700 px-3 py-2 rounded-lg">
                        <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${state.outcomeRecorded === 'partial' ? 'bg-amber-400' : 'bg-gray-500'}`} />
                    ...
                    <p className="text-[15px] text-gray-200 leading-relaxed mb-4">{item.briefing_text}</p>

                    {/* Secondary flags */}
                    {item.flags.filter(f => f.flag_type !== 'green').length > 1 && (
                      <div className="mb-4 space-y-1.5">
                        {item.flags.filter(f => f.flag_type !== 'green').slice(1, 4).map(flag => (
                          <div key={flag.id} className="flex items-start gap-2 text-xs text-gray-400">
                    ...
                    {state.selectedAction && state.selectedAction !== '__other__' ? (
                      ...
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Describe your action</p>
                        <textarea
                          autoFocus
                          ...
                          rows={2}
                          className="w-full text-sm px-3 py-2.5 border border-gray-700 bg-gray-700 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none mb-2"
                        />
                        <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 text-sm font-medium px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                          >
                            Next
                          </button>
                          <button
                            onClick={() => patch(item.customer.id, { selectedAction: undefined, otherText: undefined })}
                            className="text-sm px-4 py-2 border border-gray-700 text-gray-400 rounded-xl hover:bg-gray-700 transition-all"
                          >
                            Cancel
                          </button>
                        </div>
                      ...
                      <div>
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Pick an action</p>
                        <div className="space-y-2">
                          {item.suggested_actions.map((action, i) => (
                            <button
                              key={i}
                              onClick={() => patch(item.customer.id, { selectedAction: action })}
                              className="w-full text-left text-sm px-4 py-3 rounded-xl border border-gray-700 hover:border-blue-500 hover:bg-blue-900 hover:bg-opacity-20 text-gray-300 hover:text-blue-300 group transition-all"
                            >
                              <span className="inline-flex items-center gap-2">
                                <span className="w-5 h-5 rounded-full bg-gray-700 group-hover:bg-blue-700 text-gray-500 group-hover:text-blue-400 flex items-center justify-center text-xs font-bold flex-shrink-0">
                                  {i + 1}
                                </span>
                                {action}
                              </span>
                          ...
                          <button
                            ...
                            className="w-full text-left text-sm px-4 py-3 rounded-xl border border-dashed border-gray-700 hover:border-blue-500 hover:bg-blue-900 hover:bg-opacity-20 text-gray-500 hover:text-blue-300 group transition-all"
                          >
                            <span className="inline-flex items-center gap-2">
                              <span className="w-5 h-5 rounded-full bg-gray-700 group-hover:bg-blue-700 text-gray-600 group-hover:text-blue-400 flex items-center justify-center text-xs font-bold flex-shrink-0">
                                +
                              </span>
      ...
      <div className="mt-8 pt-6 border-t border-gray-700 flex items-center justify-between">
        ...
        <Link href="/overview" className="text-xs text-gray-400 hover:text-blue-400 font-medium">
          View all accounts →
        </Link>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex items-start gap-2.5 mb-4">
        <div className="w-5 h-5 rounded-full bg-amber-900 flex items-center justify-center flex-shrink-0 mt-0.5">
          <svg className="w-3 h-3 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        </div>
        <div>
          <p className="text-sm font-medium text-white">This flag resurfaces — did the last action help?</p>
          <p className="text-xs text-gray-400 mt-0.5">
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className="flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-xl border border-emerald-700 bg-emerald-900 text-emerald-300 hover:bg-emerald-800 font-medium transition-all"
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-xl border border-amber-700 bg-amber-900 text-amber-300 hover:bg-amber-800 font-medium transition-all"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-xl border border-gray-700 bg-gray-700 text-gray-400 hover:bg-gray-600 font-medium transition-all"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function CommitForm({
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2 text-sm text-blue-300 bg-blue-900 bg-opacity-30 px-3 py-2.5 rounded-lg">
      ...
      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Check impact in</p>
            ...
            <button
              ...
              className={`text-sm px-4 py-2 rounded-xl border font-medium transition-all ${
                checkBackDate === presetDate
                  ...
                  : 'border-gray-700 text-gray-400 hover:border-gray-600 hover:bg-gray-700'
              }`}
            >
              {opt.label}
            </button>
      ...
      <input
        ...
        className="w-full text-sm px-3 py-2 border border-gray-700 bg-gray-700 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4"
      />

      <div className="flex gap-2">
        <button
          onClick={onCommit}
          className="flex-1 text-sm font-medium px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all"
        >
          Commit
        </button>
        <button
          onClick={onBack}
          className="text-sm px-4 py-2.5 border border-gray-700 text-gray-400 rounded-xl hover:bg-gray-700 transition-all"
        >
          Back
        </button>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 text-center">
        ...
        <p className="text-gray-200 font-medium mb-1">Nothing urgent today.</p>
        <p className="text-sm text-gray-400 mb-6">
          No accounts are flagged right now. Either everything is healthy, or you have not imported customers yet.
        </p>
        <div className="flex gap-3 justify-center">
          <Link href="/upload" className="text-sm px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Import customers
          </Link>
          <Link href="/overview" className="text-sm px-4 py-2 border border-gray-700 text-gray-300 rounded-lg hover:bg-gray-700">
            View all accounts
          </Link>
        </div>
      </div>
...
function BriefingLoading() {
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-7">
        <div className="h-3 w-32 bg-gray-700 rounded animate-pulse mb-2" />
        <div className="h-7 w-44 bg-gray-700 rounded animate-pulse" />
      </div>
      {[...Array(2)].map((_, i) => (
        <div key={i} className="bg-gray-800 border border-gray-700 rounded-2xl mb-5 overflow-hidden">
          <div className="h-20 bg-gray-700 animate-pulse" />
          <div className="p-5 space-y-3">
            <div className="h-4 bg-gray-700 rounded animate-pulse" />
            <div className="h-4 w-3/4 bg-gray-700 rounded animate-pulse" />
            <div className="h-12 bg-gray-700 rounded-xl animate-pulse mt-4" />
